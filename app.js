'use strict'

const fs = require('fs')//文件操作
const path = require('path')//处理文件路径
const mongoose = require('mongoose')//连接mongodb

const db = 'mongodb://localhost/test'

/**
 * mongoose连接数据库
 * @type {[type]}
 */
mongoose.Promise = require('bluebird')//将异步方法转换为同步方法调用
mongoose.connect(db)

/**
 * 获取数据库表对应的js对象所在的路径
 * @type {[type]}
 */
const models_path = path.join(__dirname, '/app/models')


/**
 * 已递归的形式，读取models文件夹下的js模型文件，并require
 * @param  {[type]} modelPath [description]
 * @return {[type]}           [description]
 */
var walk = function(modelPath) {
    fs
        .readdirSync(modelPath)
        .forEach(function(file) {
            var filePath = path.join(modelPath, '/' + file)
            var stat = fs.statSync(filePath)//同步获取文件信息

            if (stat.isFile()) {
                if (/(.*)\.(js|coffee)/.test(file)) {
                    require(filePath)
                }
            }
            else if (stat.isDirectory()) {
                walk(filePath)
            }
        })
}
walk(models_path)

require('babel-register')//将es6转成es5
const Koa = require('koa')
const logger = require('koa-logger')//日志中间件
const session = require('koa-session')//session管理中间件
const bodyParser = require('koa-bodyparser')//解析body的中间件

const app = new Koa()

app.keys = ['homi']
app.use(logger())
app.use(session(app))
app.use(bodyParser())


/**
 * 使用路由转发请求
 * @type {[type]}
 */
const router = require('./config/router')()

app
    .use(router.routes())// 加载路由中间件
    .use(router.allowedMethods());



app.listen(8081)
console.log('app started at port 8081...');